Task: Front-matter retrofit — makepdlf-project-library.
Scope is strictly limited to: C:\dev\dev-makepdlf\makepdlf-project-library\
...
a subsequent date-format correction pass, and for mb-3-cs-project-library
and mb4ecom-project-library — this prompt already builds in the corrected
timestamp method from the start, so no separate correction pass should be
needed afterward.

Note: this repo was previously dev-pdlf / pdlf-project-library, renamed to
dev-makepdlf / makepdlf-project-library, with content repair already run
via OpenCode (identifiers, AGENTS.md title, wip todo filename updated; one
obsolete file deliberately left untouched). This retrofit runs after that
rename and repair, on the settled result — do not re-open the rename work.
...
(C:\projects-reference\custom-skills-store\mb-align-docs\front-matter-schema.md):
  ...
  - doc-id: assigned sequentially as "makepdlf-0001", "makepdlf-0002", etc.,
...
In scope: all .md documents inside makepdlf-project-library's content
...
Excluded from this task entirely — do not add front matter, do not list:
  - Anything inside a folder named "obsolete" (retired content is outside
    front-matter scope) — this includes the one obsolete file deliberately
    left untouched during the earlier rename/repair
  - Anything inside Quarantine (excluded on principle, regardless of
    current contents)
...
Confirm the makepdlf-project-library git working tree is clean before
proceeding. If dirty, stop and report — do not proceed.

Step 2 — Backup.
Take a full backup of makepdlf-project-library before any write. Save the
